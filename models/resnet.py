...
def init_model(model):
    ...
    if model.fixup:
        for m in model.modules():
            if isinstance(m, FixupBasicBlock) or isinstance(m, BasicBlock):
                nn.init.normal_(m.conv1.weight, mean=0, std=np.sqrt(2 / (m.conv1.weight.shape[0] * np.prod(m.conv1.weight.shape[2:]))) * model.num_layers ** (-0.5))
            ...
            elif isinstance(m, FixupBottleneck) or isinstance(m, Bottleneck) :
                nn.init.normal_(m.conv1.weight, mean=0, std=np.sqrt(2 / (m.conv1.weight.shape[0] * np.prod(m.conv1.weight.shape[2:]))) * model.num_layers ** (-0.25))
                nn.init.normal_(m.conv2.weight, mean=0, std=np.sqrt(2 / (m.conv2.weight.shape[0] * np.prod(m.conv2.weight.shape[2:]))) * model.num_layers ** (-0.25))
...
def conv1x1(in_planes, out_planes, stride=1, conv_options = None):
    """1x1 convolution"""

    #return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)

    return AConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False, conv_options = conv_options)
...
class FixupBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes,  stride=1, expansion=4, downsample=None, groups=1, residual_block=None, dropout=0.0, conv_options = None):
        ...
        self.bias1a = nn.Parameter(torch.zeros(1))
        self.conv1 =  conv1x1(inplanes, planes, conv_options = conv_options)  
        self.bias1b = nn.Parameter(torch.zeros(1))
        self.bias2a = nn.Parameter(torch.zeros(1))
        ...
        self.conv3 = conv1x1(inplanes, planes*expansion, conv_options = conv_options) 
        self.scale = nn.Parameter(torch.ones(1))
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, expansion=1, stride=1, groups=1, residual_block=None, dropout=None, mixup=False, conv_options = None, fixup = False):
        downsample = None
    ...
    def __init__(self, num_classes=1000, inplanes=64,
        ...
        for i in range(len(layers)):
            layer = self._make_layer(block=block, planes=width[i], blocks=layers[i], expansion=expansion,
                                     ...
                                     mixup=mixup, conv_options = conv_options, fixup = fixup)
...
class ResNet_cifar(ResNet):
    ...
    def __init__(self, num_classes=10, inplanes=16,
                 ...
                 chunks= [1,1,1], acc_bits = [0,0,0], N = 0, M = 0, fix= 0 , fixup = False, batch = 128):
        ...
        self.inplanes = inplanes
        n = int((depth - 2) / 6)
        self.num_layers = n*3 ## for fixup

        self.conv1 =  conv3x3(3, self.inplanes, stride = 1, conv_options = conv_options)
        ...
        self.layer1 = self._make_layer(block, width[0], n, groups=groups[0],
                                       residual_block=residual_block, dropout=dropout, mixup=mixup, conv_options = conv_options, fixup = fixup)
        ...
        init_model(self) ## fixup / no fixup
        self.regime = [
        ...
        if 'fixup' in regime:
            scaler = 1.1*np.sqrt(batch/128)
            self.regime = [
                {'epoch': 0, 'optimizer': 'SGD', 'lr': scaler*1e-1, 'momentum': 0.9,
                 'regularizer': weight_decay_config(5e-4)},
                {'epoch': 60, 'lr': scaler*2e-2},
                {'epoch': 120, 'lr': scaler*4e-3},
                {'epoch': 160, 'lr': scaler*8e-4}
            ]
        ...
        if 'long' in regime:
            ...
            self.regime = [
                {'epoch': 0, 'optimizer': 'SGD', 'lr': 1e-1, 'momentum': 0.9,
                 'regularizer': weight_decay_config(5e-4)},
                {'epoch': scaler*60, 'lr': 2e-2},
                {'epoch': scaler*120, 'lr': 4e-3},
                {'epoch': scaler*160, 'lr': 8e-4}
            ]
...
def resnet(**config):
    ...
    if bn_norm is not None:
        ...
        if bn_norm == 'fixup':
            bottleneck = FixupBottleneck
            basicblock = FixupBasicBlock
            config['fixup'] = True
